                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 16) {
                        socialRow(
                            onAppleTap: {
                                Task {
                                    let wasGuest = authVM.isGuestMode || store.isGuestUser
                                    await authVM.signInWithApple()
                                    if authVM.isLoggedIn {
                                        if wasGuest, let uidStr = authVM.currentUserId, let newUid = UUID(uuidString: uidStr) {
                                            let name = authVM.userName ?? "User"
                                            let email = authVM.userEmail ?? ""
                                            if let guestUpgrade = guestUpgrade {
                                                guestUpgrade(newUid, name, email)
                                            } else {
                                                store.migrateGuestData(toUserId: newUid, name: name, email: email, authProvider: .apple)
                                    ...
                                    if authVM.isLoggedIn {
                                        if wasGuest, let uidStr = authVM.currentUserId, let newUid = UUID(uuidString: uidStr) {
                                            ...
                                            if let guestUpgrade = guestUpgrade {
                                                ...
                                                store.migrateGuestData(toUserId: newUid, name: name, email: email, authProvider: .google)
                                                authVM.upgradeGuestToUser()
                                            }
                                        }
                                        onProceed()
