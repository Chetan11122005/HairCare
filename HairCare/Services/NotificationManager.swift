...
final class NotificationManager {
    ...
    /// Cancels all existing HairCure notifications by prefix and reschedules based on current settings.
    func reschedule(settings: NotificationSettings) {
        center.getPendingNotificationRequests { [weak self] requests in
            guard let self = self else { return }
            let hcIdentifiers = requests
                .map { $0.identifier }
                .filter { id in
                    id.hasPrefix(Prefix.water) ||
                    id.hasPrefix(Prefix.meal) ||
                    id.hasPrefix(Prefix.bedtime) ||
                    id.hasPrefix(Prefix.scan)
                }
            if !hcIdentifiers.isEmpty {
                self.center.removePendingNotificationRequests(withIdentifiers: hcIdentifiers)
            }

            if settings.waterReminderEnabled {
                self.scheduleWaterReminders(intervalHours: settings.waterReminderIntervalHours)
            }
            if settings.mealReminderEnabled {
                self.scheduleMealReminders(times: settings.mealReminderTimes)
            }
            if settings.bedtimeReminderEnabled {
                self.scheduleBedtimeReminder(minutesBefore: settings.bedtimeReminderMinutesBefore)
            }
            if settings.weeklyScanReminderEnabled {
                self.scheduleWeeklyScanReminder(day: settings.weeklyScanReminderDay,
